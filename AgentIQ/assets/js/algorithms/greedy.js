/* ============================================================
   greedy.js — Greedy Best-First Search Engine (ES6 Module)
   ============================================================ */
...
export class GreedyEngine {
  ...
  reset() {
    ...
    this.state.renderData = {
      queue: [], // stores node objects: sorted by h(n) value
  ...
  step() {
    ...
    switch (rd.phase) {
      ...
      case 'insert_start':
        // line 3: frontier.insert(node, priority=h(node))
        const root = {
          state: rd.rootState,
          parent: null,
          action: null,
      ...
      case 'pop_min':
        ...
        const node = rd.queue.shift(); // lowest heuristic is first
        rd.currentNode = node;
      ...
      case 'add_frontier':
        ...
        if (!inExplored) {
          ...
          if (frontierIdx === -1) {
            // line 14: frontier.insert(child, priority=h)
            rd.queue.push(rd.activeChild);
          }
        }

        // Sort frontier priority queue by heuristic h(n)
        rd.queue.sort((a, b) => a.h - b.h);
        this.state.frontier = rd.queue.map(n => n.label);
